...
**双影密探 ShadowSleuth（智能相册清理）** 是一款面向 Android 用户的轻量化图库工具 APK。它无需联网、不执行任何删除操作，仅通过「文件名」、「文件大小」和「图片分辨率」三套规则，在本地快速找出手机里的重复图片，并以分组对比的形式展示出来，方便你一眼判断哪些图片需要后续处理。
...
- **双规则匹配**：按文件名、文件字节大小或图片分辨率（宽高）三套规则，判定疑似重复
...
- **版本标签**：[v1.0.5-debug](https://github.com/th2006464/ShadowSleuth/releases/tag/v1.0.5-debug)（新增分辨率匹配 + 结果页筛选 + 回到顶部 + 导航修复）
...
*Last updated: 2026-06-23 (v1.0.5-debug)*
